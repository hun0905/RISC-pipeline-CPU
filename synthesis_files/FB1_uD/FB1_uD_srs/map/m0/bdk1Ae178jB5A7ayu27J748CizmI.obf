...
if { ${SllOptVivadoEnable} == 1 } {
    ...
    set_param place.tdm.numTDMGroupsPerCounter $SllOptTdmPerCounter
    ...
    while { [gets $file_r line] >= 0 } {
      if { [string match "Nets with Routing Errors:" $line] } {
             # Now read until we find the stop pattern
              ...
              puts $net_error
                report_route_status -file secondpassroute.rpt 
                set file_r1 [open "secondpassroute.rpt" r]
                 seek $file_r1 0
                  while { [gets $file_r1 line] >= 0 } {
                  if { [string match "*routing errors*" $line] } { set net_error_2   [lindex $line 7 ] } 
                     }
                     puts $net_error_2
                     close $file_r1
                     if { $net_error_2 > 0 } {
...
proc hVivado_post_route_reports {} {
    ...
    if { (${SllOptVivadoEnable} == 1) && [file exists "vivado_sll_opt_proxy.xdc"]} {
        ...
        report_timing -from $sllTdmFastClk -to $sllTdmFastClk -nworst 3 -max_paths 3 -file ${DesignName}_sllopt.txt
...
proc hVivado_disconnect_ip {} {
    ...
    foreach group [array names dap_groups] {
        foreach dap_inst [serialize_vector [lindex $dap_groups($group) 0] ] {
            ...
            if { $driver_type eq "GROUND" } {
                # The dap_dPin ist probably a dummy input, because it's connected to GND
                lappend dap_dummys $dap_dPin
                    continue
            }

            set_property DONT_TOUCH false $dap_net
            disconnect_net -objects $dap_dPin
            set_property DONT_TOUCH true [get_cells -of $dap_dPin];# dont touch debug IP input
            set_property DONT_TOUCH true [get_cells -of $driverPin];# dont touch instrumented net
            set_property DONT_TOUCH true $driver_net 
...
# Reconnect all daps to user nets
    foreach {dap_dPin driverPin} $dap_connections_new {
        ...
        set driverNet [get_nets -of_objects [get_pins $driverPin]]
        if {!($FORCE_SAD_DONT_TOUCH)} {
            set_property DONT_TOUCH FALSE $driverNet
        }

        # Disconnect all dap_dPins
        # dummy dap_dPins were originally connected to GDN and still are
        # all disconnected dap_dPins were disconnected from there original input and connected to
        # a generated GND to avoid warnings and possible other problems
        set dapNet [get_nets -of_objects [get_pins $dap_dPin]]
        if {!($FORCE_SAD_DONT_TOUCH)} {
            set_property DONT_TOUCH FALSE $dapNet
        }
        disconnect_net -objects $dap_dPin
        ...
        # Connect new driver (user net) to dap_dPin
        if {[catch {[connect_net -hier -net $driverNet -objects $dap_dPin]} error]} {
            puts "Warning: Could not connect $driverNet."
            puts $error
        } else {
            ...
            #lappend old->new to list
            lappend dap_connections_suc $dap_dPin $driverPin
        }
...
proc optimize_dap_net_Connections {} {
    ...
    # For each group: check all driver pins and find nearest dap_pin --> add the fond pairing to dap_connections_new
    foreach group [array names dap_groups_nets] {
        ...
        foreach driver_pin $driver_pins {
            ...
            foreach dap_pin $dap_pins {
                set current_distance [get_pin_distance $driver_pin $dap_pin]
...
proc hVivado_generate_sllTdm_analysis {} {
    ...
    if { (${TimingSignOffSllTDMCTM} == 1) } {
        update_timing -custom_timing_models 1
...
proc exit_pwd { phase } {
    ...
    write_checkpoint -force dcp_KO_in_$phase\.dcp
    report_utilization
